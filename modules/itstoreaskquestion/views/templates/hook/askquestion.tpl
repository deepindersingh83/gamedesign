...
<div class="itstore-aq">
  ...
  {else}
    <p class="itstore-aq__empty">{l s='No questions yet — be the first to ask about this product.' mod='itstoreaskquestion'}</p>
  {/if}
  ...
  <form class="itstore-aq__form js-itstore-aq-form" method="post" action="{$aq_submit_url|escape:'html':'UTF-8'}">
    ...
    <div class="itstore-aq__form-title">{l s='Ask a Question' mod='itstoreaskquestion'}</div>
    <textarea name="question" rows="3" required placeholder="{l s='e.g. Does this include a Windows 11 licence key?' mod='itstoreaskquestion'}"></textarea>
    <input type="email" name="email" placeholder="{l s='Your email (for the reply)' mod='itstoreaskquestion'}">
    <button type="submit" class="btn btn-primary">{l s='Submit Question' mod='itstoreaskquestion'}</button>
