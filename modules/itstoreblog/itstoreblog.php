...
class Itstoreblog extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('IT Store Blog Preview');
        $this->description = $this->l('“From the Blog” 3-card preview for the home page.');
    }

    protected function defaults()
    {
        return [
            'ITSTORE_BL_TITLE' => $this->l('From the Blog'),
            'ITSTORE_BL_ALL' => '',
            'ITSTORE_BL_1_TAG' => $this->l('Buying Guide'), 'ITSTORE_BL_1_TITLE' => $this->l('How to spec a business desktop fleet in 2026'),
            ...
            'ITSTORE_BL_2_TAG' => $this->l('Gaming'), 'ITSTORE_BL_2_TITLE' => $this->l('RTX 4070 vs 4070 Ti: which belongs in your build?'),
            ...
            'ITSTORE_BL_3_TAG' => $this->l('Networking'), 'ITSTORE_BL_3_TITLE' => $this->l('Wi-Fi 6E in the office: is it worth upgrading?'),
    ...
    {
        $fields = [
            ['type' => 'text', 'label' => $this->l('Block title'), 'name' => 'ITSTORE_BL_TITLE'],
            ['type' => 'text', 'label' => $this->l('“View all” link'), 'name' => 'ITSTORE_BL_ALL'],
        ];
        for ($i = 1; $i <= self::N; $i++) {
            $fields[] = ['type' => 'text', 'label' => sprintf($this->l('Post %d — tag'), $i), 'name' => 'ITSTORE_BL_' . $i . '_TAG'];
            $fields[] = ['type' => 'text', 'label' => sprintf($this->l('Post %d — title'), $i), 'name' => 'ITSTORE_BL_' . $i . '_TITLE'];
            $fields[] = ['type' => 'text', 'label' => sprintf($this->l('Post %d — image URL'), $i), 'name' => 'ITSTORE_BL_' . $i . '_IMG'];
            $fields[] = ['type' => 'text', 'label' => sprintf($this->l('Post %d — link'), $i), 'name' => 'ITSTORE_BL_' . $i . '_LINK'];
        }

        $form = ['form' => [
            'legend' => ['title' => $this->l('Blog preview'), 'icon' => 'icon-rss'],
            'input' => $fields,
            'submit' => ['title' => $this->l('Save'), 'name' => 'submitItstoreBl'],
        ]];
