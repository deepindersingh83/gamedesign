...
{block name='page_title'}{l s='Compare products' d='Modules.Itstorecompare.Shop'}{/block}

{block name='page_content'}
  <section class="itstore-compare-page">
    {if $compare_products|@count < 1}
      <div class="itstore-compare-empty">
        <p>{l s='No products selected to compare.' d='Modules.Itstorecompare.Shop'}</p>
        <a class="btn btn-primary" href="{$urls.base_url}">{l s='Browse products' d='Modules.Itstorecompare.Shop'}</a>
      </div>
    {elseif $compare_products|@count < 2}
      <div class="itstore-compare-empty">
        <p>{l s='Add at least two products to compare them side by side.' d='Modules.Itstorecompare.Shop'}</p>
        <a class="btn btn-primary" href="{$urls.base_url}">{l s='Browse products' d='Modules.Itstorecompare.Shop'}</a>
      </div>
    {else}
      <div class="itstore-compare-scroll">
        <table class="itstore-compare-table">
          <thead>
            <tr>
              <th class="itstore-compare-table__corner">{l s='Product' d='Modules.Itstorecompare.Shop'}</th>
