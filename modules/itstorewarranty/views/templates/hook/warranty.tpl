...
<div class="itstore-warranty">
  <div class="itstore-warranty__head">
    ...
    <strong>{l s='Protect your purchase' d='Modules.Itstorewarranty.Shop'}</strong>
  </div>
  <ul class="itstore-warranty__tiers">
    {foreach from=$itstore_wr_tiers item=tier}
      <li class="itstore-warranty__tier">
        ...
        {if $tier.add_url}
          <a class="btn btn-outline-primary itstore-warranty__add" href="{$tier.add_url|escape:'html':'UTF-8'}" rel="nofollow">{l s='Add' d='Modules.Itstorewarranty.Shop'}</a>
        {/if}
      </li>
    {/foreach}
  </ul>
  <p class="itstore-warranty__note">{l s='Warranty cover applies to this product — add it to your cart together.' d='Modules.Itstorewarranty.Shop'}</p>
</div>
